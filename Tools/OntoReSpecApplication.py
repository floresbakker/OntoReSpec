...
def generate_turtle():
    ontology_iri = request.form.get('ontology_iri')
    ...
    g = Graph()
    ontology = Namespace(ontology_iri)
    respec = Namespace('https://respec.org/model/def/')

    g.add((URIRef(ontology), respec.audience, Literal(audience)))
    g.add((URIRef(ontology), respec.introduction, Literal(introduction)))
    g.add((URIRef(ontology), respec.background, Literal(background)))
    g.add((URIRef(ontology), respec.objective, Literal(objective)))
    g.add((URIRef(ontology), respec.acknowledgements, Literal(acknowledgements)))
